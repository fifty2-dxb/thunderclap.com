...
export type BlogBlock =
  ...
  // Raw sanitized HTML — used by the 544 posts imported from the legacy
  // WordPress site (see scripts/import-wp-blog.mjs). Rendered into .blog-html.
...
// 544 posts ported from the legacy thunderclap.com WordPress site, generated by
// scripts/import-wp-blog.mjs. Served at their original root-level slugs so the
// existing Google rankings carry over. Merged with the hand-written POSTS above.
import importedData from "./blog-imported.json";
export const IMPORTED_POSTS = importedData as unknown as BlogPost[];

const ALL_POSTS: BlogPost[] = [...POSTS, ...IMPORTED_POSTS];

export function getAllPosts(): BlogPost[] {
  return ALL_POSTS.slice().sort((a, b) =>
...
export function getRelatedPosts(slug: string): BlogPost[] {
  ...
  // Explicit related list wins (the hand-written posts use it).
  ...
  // Otherwise prefer same-category recent posts, then fill with anything recent.
  const rest = getAllPosts().filter((p) => p.slug !== slug);
  const sameCat = post ? rest.filter((p) => p.category === post.category) : [];
  return [...sameCat, ...rest.filter((p) => !sameCat.includes(p))].slice(0, 3);
}
